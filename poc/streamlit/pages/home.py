...
steps = [
    ("1. Upload",
     "Drop PDFs into `@DOCUMENT_STAGE` via the **Process New** page, Snowsight, or SnowSQL.",
     "`PUT file://invoice.pdf @AI_EXTRACT_POC.DOCUMENTS.DOCUMENT_STAGE`"),
    ("2. Register",
     "`SP_REGISTER_STAGED_FILES` inserts tracking rows into `RAW_DOCUMENTS`.",
     "`CALL SP_REGISTER_STAGED_FILES();`"),
    ("3. Change Detection",
     "Stream + task — pipeline only wakes when `SYSTEM$STREAM_HAS_DATA()` is true.",
     None),
    ("4. Auto-Classification",
     "Cortex AI_COMPLETE reads the first page and classifies into known types or suggests new ones. "
     "New types are auto-provisioned and flagged for admin review.",
     "`CALL SP_CLASSIFY_DOCUMENT('my_receipt.pdf');`"),
    ("5. Extraction",
     "`SP_CLASSIFY_AND_EXTRACT` classifies → looks up type-specific prompt → runs AI_EXTRACT.",
     "`CALL SP_CLASSIFY_AND_EXTRACT();`"),
    ("6. Entity + Table Extraction",
     "Structured fields land in `EXTRACTED_FIELDS`; line items go to `EXTRACTED_TABLE_DATA`.",
     None),
    ("7. Serve",
     "Views reshape the data for the dashboard, analytics, and review pages.",
     None),
]
